...
{
    internal class OhlcvHistoricalDataByExchangeCollectionProvider : BaseCollectionProvider<HistoricalOhlcvResponse>
    {
        public OhlcvHistoricalDataByExchangeCollectionProvider(
            ICryptoManager manager, Exchange exchange, Period period, DateTime startDateTime, DateTime? endDateTime)
            : base(manager)
        {
            _exchange = exchange;
            _period = period;
        ...
        private ITask<HistoricalOhlcvResponse> InternalItems(int skip = 0, int limit = 50)
        {
            ...
            if (_endDateTime == null)
            {
                task = Manager.Exchanges.Ohlcv.HistoricalAsync(
                    CancellationToken.None, _exchange, _period, _startDateTime, skip, limit);
            }
            else
            {
                task = Manager.Exchanges.Ohlcv.HistoricalAsync(
                    CancellationToken.None, _exchange, _period, _startDateTime, _endDateTime.Value, skip, limit);
            }
