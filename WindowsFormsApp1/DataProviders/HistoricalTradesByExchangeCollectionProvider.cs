...
{
    internal class HistoricalTradesByExchangeCollectionProvider : BaseCollectionProvider<HistoricalTradesResponse>
    {
        public HistoricalTradesByExchangeCollectionProvider(ICryptoManager manager,
            Exchange exchange, DateTime? startDate, DateTime? endDate) : base(manager)
        {
            _exchange = exchange;
        ...
        private ITask<HistoricalTradesResponse> InternalItems(int skip = 0, int limit = 50)
        {
            ...
            {
                task = Manager.Exchanges.Trades.HistoricalAsync(
                    CancellationToken.None, _exchange, _startDate.Value, skip, limit);
            }
            else if (_startDate != null && _endDate != null)
            {
                task = Manager.Exchanges.Trades.HistoricalAsync(
                    CancellationToken.None, _exchange, _startDate.Value, _endDate.Value, skip, limit);
            }
