...
namespace TestCryptoApiSdkProject.Blockchains.PaymentForwardings.CreateGetDelete
{
    ...
    public abstract class BaseBtcSimilarCoin : BaseTest
    {
        ...
        public void InvalidFromAddress()
        {
            ...
            Assert.AreEqual("'from' address is not valid", response.ErrorMessage);
        }
        ...
        public void InvalidToAddress()
        {
            ...
            Assert.IsNotNull(response);
            Assert.IsFalse(string.IsNullOrEmpty(response.ErrorMessage));
            Assert.AreEqual("'to' address is not valid", response.ErrorMessage);
        }
        ...
        public void InvalidCallbackUrl()
        {
            ...
            Assert.IsNotNull(response);
            Assert.IsFalse(string.IsNullOrEmpty(response.ErrorMessage));
            Assert.AreEqual("'callback' url is not valid", response.ErrorMessage);
        }
        ...
        public void InvalidWallet()
        {
            ...
            Assert.IsNotNull(response);
            Assert.IsFalse(string.IsNullOrEmpty(response.ErrorMessage));
            Assert.AreEqual($"Wallet '{wallet}' does not exist", response.ErrorMessage);
        }
        ...
        public void WeakPassword()
        {
            ...
            Assert.IsNotNull(response);
            Assert.IsFalse(string.IsNullOrEmpty(response.ErrorMessage));
            Assert.AreEqual("'password' is too weak. Min size is 10 characters, actual is 3", response.ErrorMessage);
        }
        ...
        public void InvalidPassword()
        {
            ...
            try
            {
                ...
                Assert.IsNotNull(response);
                Assert.IsFalse(string.IsNullOrEmpty(response.ErrorMessage));
                Assert.AreEqual("'wallet' can not be decrypted with this 'password'", response.ErrorMessage);
            }
            finally
            {
                var fromWalletDelete = Manager.Blockchains.Wallet.DeleteWallet<DeleteWalletResponse>(NetworkCoin, fromWallet.Wallet.Name);
