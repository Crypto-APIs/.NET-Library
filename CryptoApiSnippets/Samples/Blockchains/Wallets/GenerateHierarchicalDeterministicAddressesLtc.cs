...
namespace CryptoApiSnippets.Samples.Blockchains
{
  ...
  {
    public void GenerateHierarchicalDeterministicAddressesLtc(string walletName)
    {
      ...
      Console.WriteLine(string.IsNullOrEmpty(response.ErrorMessage)
        ? "GenerateHierarchicalDeterministicAddressesLtc executed successfully, " +
          $"{response.Wallet.Addresses.Count} addresses of \"{response.Wallet.Name}\" wallet returned"
        : $"GenerateHierarchicalDeterministicAddressesLtc error: {response.ErrorMessage}");
    }
  }
}