...
{
    internal class HistoricalTradesByBaseAssetCollectionProvider : BaseCollectionProvider<HistoricalTradesResponse>
    {
        public HistoricalTradesByBaseAssetCollectionProvider(ICryptoManager manager,
            Asset baseAsset, DateTime? startDate, DateTime? endDate) : base(manager)
        {
        ...
        private ITask<HistoricalTradesResponse> InternalItems(int skip = 0, int limit = 50)
        {
            ...
            {
                task = Manager.Exchanges.Trades.HistoricalAsync(
                    CancellationToken.None, _baseAsset, _startDate.Value, skip, limit);
            }
            else if (_startDate != null && _endDate != null)
            {
                task = Manager.Exchanges.Trades.HistoricalAsync(
                    CancellationToken.None, _baseAsset, _startDate.Value, _endDate.Value, skip, limit);
            }
